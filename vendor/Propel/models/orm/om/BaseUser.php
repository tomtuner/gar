...
abstract class BaseUser extends BaseObject  implements Persistent
{
	...
	public function setUserFirstName($v)
	{
		...
		if ($this->user_first_name !== $v) {
			...
			$this->modifiedColumns[] = UserPeer::USER_FIRST_NAME;
		}
	...
	public function setUserLastName($v)
	{
		...
		if ($this->user_last_name !== $v) {
			...
			$this->modifiedColumns[] = UserPeer::USER_LAST_NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->user_first_name = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->user_last_name = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			$this->user_dce_name = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(UserPeer::USER_FIRST_NAME)) {
			$modifiedColumns[':p' . $index++]  = '`USER_FIRST_NAME`';
		}
		if ($this->isColumnModified(UserPeer::USER_LAST_NAME)) {
			$modifiedColumns[':p' . $index++]  = '`USER_LAST_NAME`';
		}
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`USER_FIRST_NAME`':
						$stmt->bindValue($identifier, $this->user_first_name, PDO::PARAM_STR);
						break;
					case '`USER_LAST_NAME`':
						$stmt->bindValue($identifier, $this->user_last_name, PDO::PARAM_STR);
						break;
					case '`USER_DCE_NAME`':
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setUserFirstName($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setUserLastName($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setUserDceName($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UserPeer::USER_FIRST_NAME)) $criteria->add(UserPeer::USER_FIRST_NAME, $this->user_first_name);
		if ($this->isColumnModified(UserPeer::USER_LAST_NAME)) $criteria->add(UserPeer::USER_LAST_NAME, $this->user_last_name);
	...
	public function setArtRequestArtStatuss(PropelCollection $artRequestArtStatuss, PropelPDO $con = null)
	{
		...
		foreach ($artRequestArtStatuss as $artRequestArtStatus) {
			...
			if ($artRequestArtStatus->isNew()) {
				$artRequestArtStatus->setUser($this);
			}
	 ...
	 * @return     PropelCollection|array ArtRequestArtStatus[] List of ArtRequestArtStatus objects
	 */
	public function getArtRequestArtStatussJoinArtRequest($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = ArtRequestArtStatusQuery::create(null, $criteria);
	 ...
	 * @return     PropelCollection|array ArtRequestComment[] List of ArtRequestComment objects
	 */
	public function getArtRequestCommentsJoinArtRequest($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = ArtRequestCommentQuery::create(null, $criteria);
		$query->joinWith('ArtRequest', $join_behavior);
